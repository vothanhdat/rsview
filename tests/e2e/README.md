...
`tui_e2e.py` drives the real `jsonview` binary inside a pseudo-terminal and asserts
...
cargo build --release
pip install pyte
python tests/e2e/tui_e2e.py            # uses target/release/jsonview
python tests/e2e/tui_e2e.py path/to/jsonview  # or point at a specific binary
```
