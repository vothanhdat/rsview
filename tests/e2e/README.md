...
`tui_e2e.py` drives the real `rsview` binary inside a pseudo-terminal and asserts
...
cargo build --release
pip install pyte
python tests/e2e/tui_e2e.py            # uses target/release/rsview
python tests/e2e/tui_e2e.py path/to/rsview   # or point at a specific binary
```
